...
(defcustom highlighter-default-face nil
  "The default face used to highlight regions.
If `nil' the `match' face is used."
  :type 'face
  :package-version '(highlighter . "1.0.0")
  :group 'highlighter)

(defcustom highlighter-multiple-cursors-prompt-once-p t
  "If non-nil, only prompt for a face once when using `multiple-cursors'.
When using the `multiple-cursors' package, commands that prompt the
user for a face will prompt the user for every single cursor.  When
set to a non-nil value this option ensures that the user is only
prompted once."
  :type 'boolean
  :package-version '(highlighter . "1.0.0")
  :group 'highlighter)
...
(defvar-local highlighter--selected-face nil)

(defun highlighter--read-face-name (prompt face-predicate)
  (if (and (bound-and-true-p mc--executing-command-for-fake-cursor)
           highlighter-multiple-cursors-prompt-once-p)
      highlighter--selected-face
    (setq highlighter--selected-face nil)
    (let ((prompt (format "%s: " prompt))
      ...
      (let ((face (completing-read
        ...
        (when (facep face)
          (setq highlighter--selected-face
                (if (stringp face)
                    (intern face)
                  face)))))))
...
(defun highlighter-mark-region (bounds &optional face)
  "Highlight text in the buffer within BOUNDS using FACE.
